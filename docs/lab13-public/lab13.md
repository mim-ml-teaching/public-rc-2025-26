...
The `manipulator.xml` file contains a 2-link robot arm.
The robot arm is controlled by two joints.
The first joint is fixed to the world and the second joint is fixed to the first link.
The robot arm is shown in the figure below.
...
Draw the robot arm on paper and calculate the forward kinematics of the robot arm.
The forward kinematics of the robot arm is a function that maps the joint angles to the position of the robot tip.

### Subtask 2:

In the `2d_manipulator_ik.py` file, we have already implemented the main loop of inverse kinematics.
It uses the `mujoco.mj_jac` function for computing the Jacobian of the robot tip, i.e.,
the matrix that maps the joint velocities to the velocity of the tip.
In our case it is a 2x2 matrix.
To get the relation between the velocities we have to look at its columns:
the first describes the velocity in the x direction and the second in the y direction.
...
The `manipulator3d.xml` file contains a 3-link robot arm.
The robot arm is controlled by three joints,
connected in sequence.
The robot arm is shown in the figure below.

![3-link robot arm](manipulator3d-start.png)

We recommend starting with the following stub code,
since at the beginning the robot arm will be in a configuration
where there is singularity in the Jacobian.
...
Take the configuration from `manipulator3d.xml`
and replace the first joint (`t1`) with a prismatic joint (`slide`` in MuJoCo).
Write the forward kinematics for the new configuration.
