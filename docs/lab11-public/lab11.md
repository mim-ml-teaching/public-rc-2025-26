...
In this class, you will implement a basic [Kalman filter](https://en.wikipedia.org/wiki/Kalman_filter) along with a mouse-tracking example similar to [this one](https://docs.google.com/presentation/d/e/2PACX-1vSz1_lcR62_W_70RWyGCPF-Fo27qdCqLja3BiG2DSFc6O-GOUDKi9tNskNLJgnpWl0bnpCcE5ucM9I4/pub?start=false&loop=false&delayms=3000#slide=id.g1ec40388a3a_0_17).
You already had an opportunity to gain some intuition about filters in [lab 8](https://mim-ml-teaching.github.io/public-rc-2025-26/lab8-public/lab8.html).
...
Kalman filter is used to approximate true state of the world given noisy observations (e.g. sensor noise). For the sake of clarity, *a priori* estimates are calculated before the measurement, while *a posteriori* after measurement update.

Some notation: at timestep \\(k\\):
- \\(z_k\\) denotes the observation
- \\(t_k\\) denotes the true state of the world
- \\(x_k\\) denotes the *a posteriori* estimate of the state of the world; it's sometimes denoted \\(\\hat{x}_{k,k}\\)
- \\(P_k\\) denotes the *a posteriori* covariance of the estimate of the state of the world; it's sometimes denoted \\(\\hat{P}_{k,k}\\)
...
- $ x'_k $ denotes the *a priori* estimate of the state of the world; it's sometimes denoted \\(\\hat{x}_{k,k-1}\\)
- $ P'_k $ denotes the *a priori* covariance of the estimate of the state of the world; it's sometimes denoted \\(\\hat{P}_{k,k-1}\\)
...
Since we can only observe position, our measurement matrix is
```
H = [1, 0]
``` 
...
You are given a kalman.py file with a partialy implemented Kalman filter. Once again, we will use the constant velocity model; however, in this task, we will extend it to two-dimensional space.
